...
interface Props {
  ...
  /** GPS accuracy in metres — draws a blue ring showing the uncertainty area */
...
const IssueMap = ({
  ...
  useEffect(() => {
    ...
    // Accuracy radius circle — remove old, add new
    if (accuracyCircle.current) {
      accuracyCircle.current.remove();
      accuracyCircle.current = null;
    }
    if (userLocation && isValidCoord(userLocation.lat, userLocation.lng) && locationAccuracy && locationAccuracy > 0) {
      accuracyCircle.current = L.circle([userLocation.lat, userLocation.lng], {
  ...
  }, [issues, userLocation, locationAccuracy, severityFilter, statusFilter, onSelectIssue]);
