...
import {
  ...
  Check, MessageCircle, Send, TrendingUp, ChevronRight,
} from "lucide-react";
...
const screenWrap = "absolute inset-0 pt-10 pb-20 px-4 overflow-hidden bg-[oklch(0.98_0.01_245)]";
      ...
      >
        <div className="flex items-center gap-1.5 mb-2">
          ...
          <div className="ml-auto flex gap-1">
            <span className="text-[10px] bg-civic-red text-white px-2 py-0.5 rounded-full font-medium">Unsolved</span>
        ...
        <div className="flex gap-1.5 mb-2 overflow-hidden">
          {[["My Ward", 1], ["City", 14], ["District", 14]].map(([l, n]) => (
            <span key={l as string} className="text-[10px] bg-white border border-border px-2 py-1 rounded-full whitespace-nowrap">
              {l} <b className="ml-1 bg-civic-navy text-white px-1.5 rounded-full">{n}</b>
            </span>
          ))}
        </div>
        ...
        {[
          ...
          >
            <div className="w-12 h-12 rounded-lg bg-gradient-to-br from-civic-blue-soft to-civic-blue" />
            <div className="flex-1">
              <div className="flex items-center gap-1">
                <span className={`text-[9px] px-1.5 py-0.5 rounded-full text-white font-medium ${it.sev === "Severe" ? "bg-civic-red" : "bg-civic-amber"}`}>{it.sev}</span>
                <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-muted text-civic-navy font-medium">Open</span>
              </div>
    ...
    >
      ...
      >
        ...
        <motion.div
          ...
          className="bg-civic-blue text-white text-xs font-semibold px-5 py-2.5 rounded-full flex items-center gap-2"
        >
...
function CaptureScreen() {
  return (
    ...
    >
      ...
      <motion.div
        initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }}
        className="mt-4 bg-white rounded-2xl p-4 border border-border"
      >
        <div className="font-semibold text-sm mb-2">Take a Photo</div>
        <div className="relative aspect-square rounded-xl overflow-hidden bg-gradient-to-br from-civic-navy via-civic-blue to-civic-blue-soft">
          <motion.div
            initial={{ y: "-100%" }}
            animate={{ y: "100%" }}
          ...
          <div className="absolute inset-0 grid place-items-center">
            <motion.div
              ...
              className="w-12 h-12 rounded-full bg-civic-green flex items-center justify-center"
            >
        ...
        <motion.div
          ...
          transition={{ duration: 1.4, repeat: Infinity }}
          className="mt-3 bg-civic-blue text-white text-xs font-semibold py-2.5 rounded-full flex items-center justify-center gap-2"
        >
...
function SubmitScreen() {
  return (
    ...
    >
      ...
      <motion.div
        initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }}
        className="mt-4 bg-white rounded-2xl p-4 border border-border space-y-2.5"
      >
        <div className="font-semibold text-sm">Review & Submit</div>
        {[
          ["Category", "Electric Cable Issue"],
          ["Department", "Electricity Dept."],
          ["Ward", "Hosur"],
          ["Location", "12.7594, 77.8129"],
        ].map(([k, v]) => (
        ...
        <div className="aspect-video rounded-lg bg-gradient-to-br from-civic-blue-soft to-civic-blue" />
        <motion.div
          animate={{ scale: [1, 1.04, 1] }}
          transition={{ duration: 1.2, repeat: Infinity }}
          className="bg-civic-blue text-white text-xs font-semibold py-2.5 rounded-full text-center"
        >
          Submit Report
        </motion.div>
      </motion.div>
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        className="mt-4 bg-white rounded-2xl p-4 border border-border"
      >
        <div className="aspect-video rounded-lg bg-gradient-to-br from-civic-blue-soft to-civic-blue mb-3" />
        <div className="flex items-center gap-1 mb-1">
          <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-civic-red text-white font-medium">Severe</span>
      ...
      <motion.div
        ...
        className="mt-4 bg-civic-green/15 border border-civic-green/40 rounded-full px-3 py-2 flex items-center gap-2"
      >
        <div className="w-5 h-5 rounded-full bg-civic-green flex items-center justify-center">
          <Check className="w-3 h-3 text-white" strokeWidth={4} />
        </div>
        <span className="text-xs font-medium text-civic-navy">Report submitted successfully!</span>
      </motion.div>
    </motion.div>
  );
}
...
function CommentsScreen() {
  return (
    ...
    >
      ...
      <motion.div
        initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }}
        className="mt-3 bg-white rounded-2xl p-3 border border-border flex gap-2"
      >
        <motion.button
          whileTap={{ scale: 0.95 }}
          animate={{ backgroundColor: ["oklch(0.62 0.18 245 / 0.15)", "oklch(0.62 0.18 245 / 0.35)", "oklch(0.62 0.18 245 / 0.15)"] }}
          transition={{ duration: 1.6, repeat: Infinity }}
          className="flex-1 text-xs font-semibold py-2 rounded-full text-civic-blue flex items-center justify-center gap-1"
        >
      ...
      <motion.div
        ...
        className="mt-3 bg-white rounded-2xl border border-border p-3"
      >
        <div className="flex items-center justify-between mb-2">
          ...
          <span className="text-[10px] text-muted-foreground">Pothole</span>
        </div>
        <div className="flex gap-1.5 mb-2">
          <div className="w-6 h-6 rounded-full bg-civic-blue text-white grid place-items-center text-[10px] font-bold">J</div>
        ...
        >
          <div className="bg-civic-blue text-white rounded-2xl px-3 py-1.5 text-[11px]">Yes, there is an issue</div>
          <div className="w-6 h-6 rounded-full bg-civic-navy text-white grid place-items-center text-[10px] font-bold">A</div>
        </motion.div>
      ...
      {[1, 2, 3, 4].map((n) => (
        <div key={n} className="flex flex-col items-center gap-1 flex-1">
          <div className={`w-7 h-7 rounded-full grid place-items-center text-[11px] font-bold ${
            n <= step ? "bg-civic-blue text-white" : "bg-muted text-muted-foreground"
          }`}>
            {n < step ? <Check className="w-3.5 h-3.5" strokeWidth={3} /> : n}
          </div>
          <span className="text-[9px] text-muted-foreground">
            {["Location", "Capture", "Details", "Submit"][n - 1]}
          </span>
        </div>
      ))}
    </div>
...
function BottomNav({ active }: { active: Scene }) {
  const items = [
  ...
  return (
    <div className="absolute bottom-0 inset-x-0 bg-white border-t border-border px-3 py-2 flex items-center justify-around">
      {items.map(({ icon: Icon, label, scenes, primary }) => {
        ...
        if (primary) {
          return (
            <div key={label} className="-mt-6">
              <div className="w-12 h-12 rounded-full bg-civic-blue grid place-items-center shadow-lg shadow-civic-blue/40">
        ...
        return (
          <div key={label} className={`flex flex-col items-center gap-0.5 ${isActive ? "text-civic-blue" : "text-muted-foreground"}`}>
